...
# Reading the first Feather file (machine1.feather)
data_machine1 = pd.read_feather('machine1.feather')

# Reading the second Feather file (machine2.feather)
...
def train_model(data, machine_type):
    ...
    # Predicting power usage for the test set
    y_pred = model.predict(X_test)

    # Calculating Mean Squared Error as a metric of accuracy
...
def calculate_total_power(model1, model2, GPH_values_machine1, GPH_values_machine2):
    # Converting numpy arrays to DataFrames with correct column names
    ...
    # Calculateing power for each machine using the trained models
    power_machine1 = model1.predict(df_machine1)
    power_machine2 = model2.predict(df_machine2)

    # Calculating total power for all machines
    total_power = sum(power_machine1) + sum(power_machine2)

    return total_power

# Initializing variables to store the optimal GPH values and total power
optimal_GPH_machine1 = 0
optimal_GPH_machine2 = 0
lowest_total_power = float('inf')

# Iterating through all possible GPH values for each machine type and find the optimal configuration
for GPH_machine1 in range(180, 601):
    for GPH_machine2 in range(300, 1001):
        # Introducing randomness to initial GPH values for each machine type
...
# Printing the optimal GPH values and total power
...
# Optimization evaluation. Calculating total power consumption before optimization
total_power_before_optimization = calculate_total_power(model_machine1, model_machine2, [400] * 10, [625] * 10)

# Calculating total power consumption after optimization
